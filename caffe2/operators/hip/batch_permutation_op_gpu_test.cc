...
template <>
void AddInput<float, HIPContext>(
    const vector<int64_t>& shape,
    const vector<float>& values,
    const string& name,
    Workspace* ws) {
...
template <>
DeviceTypeProto GetDeviceType<HIPContext>() {
  return PROTO_HIP;
}
...
void CheckCPUGPUEqual(vector<int64_t> shape, vector<int> indices) {
  ...
  if (!caffe2::HasHipGPU()) {
    VLOG(2) << "No HipGPU found. Skip GPU test." << std::endl;
    return;
  }
  ...
  CreateAndRunGradient<HIPContext>(&y_gpu_grad, N, shape, features, indices);
