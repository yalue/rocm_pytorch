...
TEST(TensorConstruction, ReinitializeTensorTest) {
  if (!HasHipGPU()) return;
  Tensor x = caffe2::empty({1}, at::dtype<float>().device(HIP, 0));
  ...
  ReinitializeTensor(&x, {1}, at::dtype<float>().device(HIP));
...
TEST(TensorTest, TensorSerializationMultiDevices) {
  ...
  for (int gpu_id = 0; gpu_id < NumHipDevices(); ++gpu_id) {
    HIPGuard guard(gpu_id);
    HIPContext context(gpu_id); // switch to the current gpu
    blob.Reset(new Tensor(tensor, HIP));
    string serialized = SerializeBlob(blob, "test");
    BlobProto proto;
    CAFFE_ENFORCE(proto.ParseFromString(serialized));
    EXPECT_EQ(proto.name(), "test");
