...
nccl::NCCLExecution getNCCLElements(
  ...
  for (auto i = 0; i < op->InputSize(); ++i) {
    ...
    el.src = &(op->Input<Tensor>(i, HIP));
    if (op->OutputSize() == 1) {
      // Reduce op
      if (i == ex.root) {
        el.dst = op->Output<Tensor>(0, HIP);
      }
    } else if (i < op->OutputSize()) {
      el.dst = op->Output<Tensor>(i, HIP);
    }
    // TODO - expensive (>1ms) - cache these.
    el.device = GetGPUIDForPointer(op->Input<Tensor>(i, HIP).raw_data());
  }
...
bool AllInputsAre(OperatorBase* op) {
  for (auto i = 0; i < op->InputSize(); ++i) {
    if (op->Input<Tensor>(i, HIP).IsType<T>()) {
      continue;
    } else {
      return false;
    }
...
class NCCLBaseOp : public Operator<HIPContext> {
 public:
...
REGISTER_HIP_OPERATOR(NCCLAllreduce, NCCLAllreduceOp);
...
REGISTER_HIP_OPERATOR(NCCLBroadcast, NCCLBroadcastOp);
...
REGISTER_HIP_OPERATOR(NCCLReduce, NCCLReduceOp);
...
REGISTER_HIP_OPERATOR(NCCLAllGather, NCCLAllGatherOp);
...
REGISTER_HIP_OPERATOR(NCCLReduceScatter, NCCLReduceScatterOp);
