...
struct HostAllocator
{
  ...
  void emptyCache()
  {
    ...
    for (const auto & cuda_event : cuda_events) {
      ...
      if (!block.allocated) {
        C10_HIP_CHECK_WARN(hipEventDestroy(event));
        block.event_count--;
      }
    }
    ...
    for (auto it = blocks.begin(); it != blocks.end();) {
      Block& block = it->second;
      if (!block.allocated) {
        C10_HIP_CHECK_WARN(hipHostFree(block.ptr));
        it = blocks.erase(it);
      } else {
        ++it;
      }
...
hipError_t CachingHostAllocator_recordEvent(void *ptr, at::hip::HIPStreamMasqueradingAsCUDA stream)
{
...
struct CachingHostAllocator final : public at::Allocator {
  at::DataPtr allocate(size_t size) const override {
    void *ptr;
    C10_HIP_CHECK(allocator.malloc(&ptr, size));
