...
void CUDAGraph::capture_begin(MempoolId_t pool/*=0*/) {
  ...
  // When HIPCachingAllocator allocates while a capture is underway, it calls cudaStreamGetCaptureInfo
  // to get the current stream's capture id, if any. Here we tell HIPCachingAllocator: if the stream
  ...
  c10::hip::HIPCachingAllocator::notifyCaptureBegin(capture_dev_, id_, mempool_id_);
#else
  TORCH_CHECK(false, "CUDA graphs may only be used in Pytorch built with CUDA >= 11.0");
#endif
}
...
void CUDAGraph::capture_end() {
  ...
  c10::hip::HIPCachingAllocator::notifyCaptureEnd(capture_dev_, id_);
...
void CUDAGraph::reset() {
  ...
  if (has_graph_ || has_graph_exec_) {
    ...
    c10::hip::HIPCachingAllocator::notifyCaptureDestroy(capture_dev_, mempool_id_);
  }
  if (has_graph_) {
