...
#if defined(HIP_VERSION) && HIP_VERSION >= 99999
...
static inline void sort_keys(
  ...
  if (descending) {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceRadixSort::SortKeysDescending,
      keys_in_, keys_out_, n,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  } else {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceRadixSort::SortKeys,
      keys_in_, keys_out_, n,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  }
}
...
static inline void sort_pairs(
  ...
  if (descending) {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceRadixSort::SortPairsDescending,
      keys_in_, keys_out_, values_in, values_out, n,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  } else {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceRadixSort::SortPairs,
      keys_in_, keys_out_, values_in, values_out, n,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  }
}
...
static inline void segmented_sort_pairs(
  ...
  if (descending) {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceSegmentedRadixSort::SortPairsDescending,
      keys_in_, keys_out_, values_in, values_out,
      num_elements, num_segments, begin_offsets, end_offsets,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  } else {
    CUB_WRAPPER(NO_ROCM(detail)::hipcub::DeviceSegmentedRadixSort::SortPairs,
      keys_in_, keys_out_, values_in, values_out,
      num_elements, num_segments, begin_offsets, end_offsets,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  }
...
inline void inclusive_scan(InputIteratorT input, OutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  ...
  for (int64_t i = max_cub_size; i < num_items; i += max_cub_size) {
    ...
    impl::transform_vals<<<1, 1, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
        ...
        scan_op);
    C10_HIP_KERNEL_LAUNCH_CHECK();
    using ArgIndexInputIterator = NO_ROCM(detail)::hipcub::ArgIndexInputIterator<InputIteratorT>;
    ...
    auto input_ = NO_ROCM(detail)::hipcub::TransformInputIterator<input_t, decltype(input_iter_transform), ArgIndexInputIterator>(
