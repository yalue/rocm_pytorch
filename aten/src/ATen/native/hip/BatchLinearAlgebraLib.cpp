...
rocblas_operation to_cublas(TransposeType trans) {
  switch (trans) {
    case TransposeType::NoTranspose: return rocblas_operation_none;
    case TransposeType::Transpose: return rocblas_operation_transpose;
    case TransposeType::ConjTranspose: return rocblas_operation_conjugate_transpose;
  }
...
inline static void _apply_single_inverse_helper(scalar_t* self_ptr, scalar_t* self_inv_ptr, int* ipiv_ptr, int* info_getrf_ptr, int* info_getrs_ptr, int n, int lda) {
  ...
  at::cuda::solver::getrs<scalar_t>(handle, n, n, self_ptr, lda, ipiv_ptr, self_inv_ptr, lda, info_getrs_ptr, rocblas_operation_none);
}
...
inline static void apply_cholesky_cusolver_potrf_looped(const Tensor& self_working_copy, bool upper, const Tensor& infos) {
  auto handle = at::cuda::getCurrentCUDASolverDnHandle();
  const auto uplo = upper ? rocblas_fill_upper : rocblas_fill_lower;
...
static void apply_ormqr(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
  ...
  auto trans = transpose ? (input.is_complex() ? rocblas_operation_conjugate_transpose : rocblas_operation_transpose) : rocblas_operation_none;
